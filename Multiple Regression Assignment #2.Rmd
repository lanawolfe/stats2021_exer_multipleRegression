...
This R markdown document provides an example of performing a multiple regression using the lm() function in R and compares the output with the linReg() function in the jmv (Jamovi) package.
...
Multiple regression is predicting a continuous outcome variable (dependent variable) with several predictor variables (independent variables). You can perform regressions using categorical predictor variables, but we'll talk more about that later.
